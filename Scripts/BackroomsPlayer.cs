...
public class BackroomsPlayer : MonoBehaviour
{
    ...
    // Added a reference to the vitals script
    private BackroomsPlayerVitals vitals;
    ...
    private void Awake()
    {
        ...
        // Grab the vitals component so we can check stamina levels
        vitals = GetComponent<BackroomsPlayerVitals>();
    ...
    private void Move()
    {
        ...
        // Check if we have the vitals script, and if so, see if we are allowed to sprint
        bool canSprintNow = (vitals == null) || vitals.CanSprint;

        // Apply the sprint check here
        IsSprinting = !IsCrouching && input.z > 0.1f && Keyboard.current != null && Keyboard.current.leftShiftKey.isPressed && canSprintNow;
